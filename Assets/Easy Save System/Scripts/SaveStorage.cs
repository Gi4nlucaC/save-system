    ...
    public static class SaveStorage
    {
        ...
        public static FileInfo[] CheckSaves(string extension)
        {
            if (!Directory.Exists(_rootPath))
                throw new DirectoryNotFoundException($"Directory '{_rootPath}' does not exist.");
            DirectoryInfo dir = new(_rootPath);
            return dir.GetFiles($"*.{extension}", SearchOption.TopDirectoryOnly);
        }

        static string PathFor(string slotId, string extension) => Path.Combine(_rootPath, $"{slotId}.{extension}");
        ...
        public static void Write(string slotId, string content) => File.WriteAllText(PathFor(slotId, "sav"), content);
        public static async Task WriteAsync(string slotId, byte[] content) => await File.WriteAllBytesAsync(PathFor(slotId, "bin"), content);
        public static async Task WriteAsync(string slotId, string content) => await File.WriteAllTextAsync(PathFor(slotId, "sav"), content);
        ...
        public static string ReadJson(string slotId) => File.Exists(PathFor(slotId, "sav")) ? File.ReadAllText(PathFor(slotId, "sav")) : null;
        ...
        public static bool Exists(string slotId, string extension) => File.Exists(PathFor(slotId, extension));
        ...
        public static void WriteJsonWithHeader(string slotId, MetaData header, List<PureRawData> datas)
        {
            ...
            File.WriteAllText(PathFor(slotId, "sav"), json);
        }
