...
public class UIManager : MonoBehaviour
{
    ...
    void OnMenuOpenClicked(){ Time.timeScale = 0; _menuPanel.SetActive(true); }
    void OnMenuCloseClicked(){ Time.timeScale = 1; _menuPanel.SetActive(false); }

    void OnSavedSlotMenuCloseClicked()
    {
    ...
    void OnGameSavedSucessfully()
    { _savedSlotsPanel.RefreshList(UISlotsStates.OVERWRITE); ShowMessage("Game Saved!"); }
    ...
    { ShowMessage("Game Saved. Quitting..."); SaveSystemManager.OnResetGame(); SceneManager.LoadScene(0); }

    void OnLoadButtonClicked()
    { SaveSystemManager.OnAllSavablesLoaded += OnGameLoadedSucessfully; _savedSlotsPanel.RefreshList(UISlotsStates.LOAD); _savedSlotsPanel.ToggleVisibility(); }

    void OnGameLoadedSucessfully()
    { SaveSystemManager.OnAllSavablesLoaded -= OnGameLoadedSucessfully; Time.timeScale = 1; }

    void OnQuitButtonClicked()
    { ShowMessage("Quitting Game..."); SceneManager.LoadScene(0); }

    void ShowMessage(string message)
    {
        if (_currentMessageRoutine != null) StopCoroutine(_currentMessageRoutine);
    ...
    IEnumerator ShowMessageRoutine(string message)
    {
        ...
        Color c = _savingLoadingText.color; c.a = 1f; _savingLoadingText.color = c;
    ...
    void OnAutoSave() => ShowMessage("Saving... Dont Turn OFF!");
}
}
